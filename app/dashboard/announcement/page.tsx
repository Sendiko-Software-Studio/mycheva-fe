...
export default function AnnouncementHome() {
  ...
  return (
    <div className="h-screen gap-16">
      {loading && (
        <p className={`transition-opacity duration-300 ease-in-out ${loading ? 'opacity-100' : 'opacity-0'} text-neutral-50 bg-primary-500 p-2 w-full rounded-lg`}>
          Loading...
        </p>
      )}
      {error && (
        <p className={`transition-opacity duration-300 ease-in-out ${error ? 'opacity-100' : 'opacity-0'} text-neutral-50 bg-error p-2 rounded-lg w-full`}>
          {error}
        </p>
      )}
