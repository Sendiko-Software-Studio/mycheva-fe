...
export default function PostAnnouncement() {
  ...
  return (
    <div className="lg:mx-24 md:mx-16 sm:mx-0 flex flex-col gap-8">
      ...
      {message && (
        <p
          className={`transition-opacity duration-300 ease-in-out ${message ? "opacity-100" : "opacity-0"} text-neutral-50 bg-green p-2 rounded-lg w-full`}
        >
          {message}
        </p>
      )}
      ...
      <form onSubmit={handlePost} method="post" className="w-full">
        <div className="flex flex-col gap-2 mb-4">
          ...
          <input
            ...
            name="title"
            id="title"
            className="border-2 border-l-neutral-300 text-lg w-full lg:w-1/2 p-4 rounded-md"
        ...
        <button className="text-lg py-2 px-4 bg-primary-500 shadow-md w-full text-white rounded">
          Tambahkan
        </button>
      </form>
    </div>
