...
export default function MeetingCard(
  {title, time, date, id}: {title: string, time: string, date: string, id: string}
) {
        ...
        <div className="flex flex-col w-full">
          <div className="flex flex-col bg-primary-300 p-4 rounded-xl w-full">
            ...
            {isExpanded && (
              <Link
                href={`attendance/${id}`}
                className="mt-2 text-sm rounded-xl p-2 bg-transparent border-2 border-primary-700 text-primary-700 hover:bg-primary-700 hover:text-white">Buat
                Kehadiran</Link>)}
          </div>
        </div>
      </div>
