...
export default function Pagination({ recordsTotal, page, link, onPageChange }: PaginationProps) {
  const recordsPerPage = 10;
  const first = recordsTotal === 0 ? 0 : (page - 1) * recordsPerPage + 1;
  const last = Math.min(page * recordsPerPage, recordsTotal);
  ...
  return (
    <div className="flex items-center mb-4 sm:mb-0 py-5">
      ...
      <span className="text-sm font-normal text-gray-500">
        ...
        of <span className="font-semibold text-gray-900 ">{recordsTotal}</span>
      </span>
    </div>
  );
}
